...
def IndustryPositionFact(date, df_industry_sector, path, save_his = False):
    '''
    ...
    if save_his:
        industry_tbl_his = pd.read_csv(f'''{path}/industry_tbl.csv''')
        ...
        industry_tbl_his.to_csv(f'''{path}/industry_tbl.csv''', index = False)
...
def IndustryPrioritization(date, df_industry_sector, industry_tbl, path, filterfunction = IndustryFilter_Trend, save_his = False):
    '''
    ...
    if save_his:
        industry_prioritize_his = pd.read_csv(f'''{path}/industry_prioritize.csv''')
    
        if list(industry_prioritize.columns) != list(industry_prioritize_his.columns):
            print(industry_prioritize.columns, industry_prioritize_his.columns)
            raise Exception('industry_prioritize table format does not match expactation')
            
        if date in industry_prioritize_his.closed_date.to_list(): #not in section_tbl.nextopen_date.to_list(): # to be updated
            industry_prioritize_his = industry_prioritize_his[industry_prioritize_his.closed_date != date]
            logging.info(f'''industry_prioritize_his Data on {date} is refreshed''')
            
        else:
            industry_prioritize_his = pd.concat([industry_prioritize_his, industry_prioritize])
            industry_prioritize_his.to_csv(f'''{path}/industry_prioritize.csv''', index = False)
